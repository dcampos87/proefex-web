...
import { services } from "@proefex/shared/data/landing";

export function Services() {
  return (
    <section id="servicios" className="relative mx-auto max-w-7xl scroll-mt-24 px-6 py-24 sm:py-32">
      <div className="wave-divider top-[-10rem] -z-10" aria-hidden="true" />
      <RevealOnScroll>
        <SectionHeading
      ...
      <div className="relative mt-16 grid gap-8 md:grid-cols-3">
        {services.map((service, index) => (
          <RevealOnScroll key={service.id} delay={index * 0.12} y={index === 1 ? 52 : 36}>
            <article
              className={`group flex h-full flex-col gap-5 rounded-[2rem] bg-gradient-to-br from-white/[0.13] to-white/[0.03] p-8 shadow-[0_25px_60px_rgba(0,34,84,0.35)] backdrop-blur-sm transition-all duration-500 ease-out hover:-translate-y-2 hover:from-white/[0.2] hover:to-white/[0.06] hover:shadow-[0_30px_70px_rgba(0,90,158,0.45)] ${index === 1 ? "md:translate-y-8" : ""}`}
            >
